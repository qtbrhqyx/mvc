...
//        method.invoke() //
        ...
        //获取controller上的注解
        Class<?> conClass = handler.getController().getClass();
        boolean p=false;
        if(conClass.isAnnotationPresent(Security.class)){
            String[] p1 = conClass.getAnnotation(Security.class).value();
            if(Arrays.asList(p1).contains(username)){
                p=true;
            }
        }
        if(handler.getMethod().isAnnotationPresent(Security.class)){
            String[] p2 = handler.getMethod().getAnnotation(Security.class).value();
            if(Arrays.asList(p2).contains(username)){
                p=true;
            }
        }
